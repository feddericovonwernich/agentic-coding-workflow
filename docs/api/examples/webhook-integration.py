...
class GitHubWebhookProcessor:
    ...
    async def _handle_pull_request(self, payload: Dict[str, Any], delivery_id: str) -> bool:
        """Handle pull request webhook events."""
        action = payload.get("action")
        pr_data = payload.get("pull_request", {})
        repository = payload.get("repository", {})
        
        logger.info(f"PR Event: {action} - PR #{pr_data.get('number')} in {repository.get('full_name')}")
    ...
    async def _handle_pr_updated(self, pr_info: Dict[str, Any], delivery_id: str):
        ...
        logger.info(f"PR updated: #{pr_info['number']} - new SHA: {pr_info['head_sha']}")
    ...
    async def _handle_check_run(self, payload: Dict[str, Any], delivery_id: str) -> bool:
        """Handle check run webhook events."""
        action = payload.get("action")
        check_run = payload.get("check_run", {})
        repository = payload.get("repository", {})
        
        logger.info(f"Check Run Event: {action} - {check_run.get('name')} in {repository.get('full_name')}")
    ...
    async def _handle_failed_check(self, check_info: Dict[str, Any], delivery_id: str):
        ...
        logger.warning(f"Check failed: {check_info['name']} - {check_info['conclusion']}")
    ...
    async def _handle_check_suite(self, payload: Dict[str, Any], delivery_id: str) -> bool:
    ...
    async def _handle_status(self, payload: Dict[str, Any], delivery_id: str) -> bool:
        ...
        logger.info(f"Status Event: {context} - {state} in {repository.get('full_name')}")
    ...
    async def _handle_push(self, payload: Dict[str, Any], delivery_id: str) -> bool:
        ...
        logger.info(f"Push Event: {len(commits)} commits to {ref} in {repository.get('full_name')}")
...
async def github_webhook_endpoint(
    ...
    try:
        ...
        if success:
            return JSONResponse({"status": "processed", "delivery_id": x_github_delivery})
        else:
...
async def example_outgoing_notifications():
    ...
    for notification in notifications:
        ...
        logger.info(f"Notification sent: {notification['event_type']} - {'Success' if success else 'Failed'}")
