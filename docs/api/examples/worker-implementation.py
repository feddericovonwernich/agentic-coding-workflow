...
class QueueManager(ABC):
    ...
    @abstractmethod
    async def enqueue(self, message: WorkerMessage, queue_name: str = "default") -> bool:
        """Add message to queue."""
        pass
    
    @abstractmethod
    async def dequeue(self, queue_name: str = "default") -> Optional[WorkerMessage]:
    ...
    @abstractmethod
    async def ack_message(self, message: WorkerMessage, queue_name: str = "default") -> bool:
        """Acknowledge message processing."""
        pass
    
    @abstractmethod
    async def nack_message(self, message: WorkerMessage, queue_name: str = "default") -> bool:
    ...
    async def _process_cycle(self):
        ...
        if len(messages) == 1:
            ...
            try:
                ...
                for message, success in zip(messages, results):
                    await self._handle_result(message, success)
            except Exception as e:
    ...
    async def process_message(self, message: WorkerMessage) -> bool:
        ...
        try:
            ...
            if random.random() < 0.8:  # 80% success rate
                ...
                logger.info(f"Analysis complete for PR {pr_id}: {analysis_result['issues_found']} issues")
                return True
            else:
...
class BatchProcessingWorker(BaseWorker):
    ...
    async def process_batch(self, messages: List[WorkerMessage]) -> List[bool]:
        ...
        logger.info(f"Batch processing complete: {sum(results)}/{len(results)} successful")
        return results
...
async def example_multi_worker_pipeline():
    ...
    for worker_name, stats in final_stats.items():
        logger.info(f"  {worker_name}: {stats['processed_count']} processed, {stats['error_count']} errors")
