...
from src.config.loader import load_config, ConfigurationLoader
from src.config.manager import ConfigurationManager
from src.config.models import (
    Config, DatabaseConfig, GitHubConfig, LLMConfig,
    AnthropicConfig, NotificationConfig, NotificationChannelConfig
)
...
"""
        ...
        try:
            ...
            if config1.system.environment != config2.system.environment:
                differences.append(
                    f"Environment: {config1.system.environment} -> {config2.system.environment}"
                )
            
            if config1.system.log_level != config2.system.log_level:
                differences.append(
                    f"Log level: {config1.system.log_level} -> {config2.system.log_level}"
                )
            
            if config1.database.pool_size != config2.database.pool_size:
                differences.append(
                    f"Pool size: {config1.database.pool_size} -> {config2.database.pool_size}"
                )
            ...
            if config1.github.timeout != config2.github.timeout:
                differences.append(
                    f"GitHub timeout: {config1.github.timeout} -> {config2.github.timeout}"
                )
