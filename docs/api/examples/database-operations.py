    ...
    async def example_basic_crud_operations(self):
        ...
        async with self.manager.get_session() as session:
            try:
                ...
                logger.info(
                    f"Created repository: {repository.full_name} (ID: {repository.id})"
                )
                ...
                logger.info(
                    f"Updated repository stars: {updated_repo.repo_metadata['stars']}"
                )
        ...
        async with self.manager.get_session() as session:
            try:
                ...
                for entry in history:
                    logger.info(
                        f"  {entry.old_state} -> {entry.new_state} "
            ...
            try:
                ...
                for config in check_configs:
                    check_run = await check_repo.create(
                        ...
                        if config["status"] == CheckStatus.COMPLETED
                        else None,
                        check_metadata={
                ...
                security_check = next(
                    c for c in created_checks if c.name == "Security Scan"
                )
...
async def comprehensive_database_example():
    ...
    try:
        ...
        await examples.example_pull_request_lifecycle()
