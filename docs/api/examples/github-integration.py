...
from src.github.rate_limiting import RateLimiter
...
class GitHubIntegrationExamples:
        ...
        try:
            ...
            for pr in pulls[:3]:  # Process first 3 PRs
                ...
                for check in check_runs[:2]:  # Show first 2 checks
                    logger.info(f"    * {check['name']}: {check['status']} - {check['conclusion']}")
    ...
    async def example_pagination_handling(self):
        ...
        try:
            ...
            for i in range(0, len(all_repos), batch_size):
                ...
                logger.info(f"Processing batch {i//batch_size + 1} with {len(batch)} repos")
    ...
    async def example_rate_limiting(self):
        ...
        try:
            ...
            logger.info(f"  Core: {rate_limit['core']['remaining']}/{rate_limit['core']['limit']}")
            logger.info(f"  Search: {rate_limit['search']['remaining']}/{rate_limit['search']['limit']}")
            ...
            for repo_name in repos_to_check:
                ...
                try:
                    ...
                    # Make API call
                    repo_data = await self.client.get_repo(owner, repo)
                    logger.info(f"Checked {repo_name}: {repo_data['stargazers_count']} stars")
