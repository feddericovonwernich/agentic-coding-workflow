...
from src.models import CheckConclusion, CheckRun, CheckStatus, PullRequest
...
class CheckRunRepository(BaseRepository[CheckRun]):
    ...
    async def get_by_external_id(self, external_id: str) -> Optional[CheckRun]:
    ...
    async def get_by_pr_and_check_name(
        ...
        query = (
            ...
            .where(
                and_(
                    CheckRun.pr_id == pr_id,
                    CheckRun.check_name == check_name
                )
            )
    ...
    async def get_recent_failures(
        ...
        since = datetime.now(timezone.utc).replace(hour=datetime.now(timezone.utc).hour - hours)
    ...
    async def get_actionable_failures(
        self,
        limit: Optional[int] = None
    ) -> list[CheckRun]:
    ...
    async def get_checks_by_category(
        ...
        return [
            ...
            if check.get_failure_category() == category
        ]
    ...
    async def get_check_statistics(
        ...
        for status in CheckStatus:
            ...
            count_query = select(func.count(CheckRun.id)).where(and_(*status_conditions))
        ...
        for conclusion in CheckConclusion:
            conclusion_conditions = conditions + [
                CheckRun.status == CheckStatus.COMPLETED,
                CheckRun.conclusion == conclusion
            ]
            count_query = select(func.count(CheckRun.id)).where(and_(*conclusion_conditions))
            result = await self.session.execute(count_query)
    ...
    async def get_check_duration_stats(
        ...
        return {
            ...
            "avg_duration_seconds": float(row.avg_duration) if row and row.avg_duration else 0.0,
            "min_duration_seconds": float(row.min_duration) if row and row.min_duration else 0.0,
            "max_duration_seconds": float(row.max_duration) if row and row.max_duration else 0.0,
        }