...
class PullRequestRepository(BaseRepository[PullRequest]):
    ...
    async def get_by_repo_url_and_number(
        ...
        query = (
            select(PullRequest)
            .join(Repository)
            .where(and_(Repository.url == repo_url, PullRequest.pr_number == pr_number))
            .options(
                selectinload(PullRequest.repository),
                selectinload(PullRequest.check_runs),
                selectinload(PullRequest.state_history),
    ...
    async def get_active_prs_for_repo(
        self, repository_id: uuid.UUID, include_drafts: bool = False
    ) -> list[PullRequest]:
        ...
        if not include_drafts:
            conditions.append(~PullRequest.draft)
    ...
    async def get_prs_needing_check(
        self, last_checked_before: datetime, limit: int | None = None
    ) -> list[PullRequest]:
    ...
    async def update_state(
        ...
        if metadata:
            ...
            # Tell SQLAlchemy the JSONB field was modified
            flag_modified(pr, "pr_metadata")
    ...
    async def mark_as_checked(self, pr_id: uuid.UUID) -> PullRequest:
        ...
        pr = await self.get_by_id_or_raise(pr_id)
        pr.last_checked_at = datetime.now(UTC)
        await self.flush()
        await self.refresh(pr)
        return pr

    async def get_prs_with_failed_checks(
        self, repository_id: uuid.UUID | None = None, limit: int | None = None
    ) -> list[PullRequest]:
        ...
        failed_checks_subquery = (
            ...
            .where(
                and_(CheckRun.status == "completed", CheckRun.conclusion == "failure")
            )
            .distinct()
        )
    ...
    async def get_recent_prs(
        ...
        conditions = [
            or_(PullRequest.created_at >= since, PullRequest.updated_at >= since)
        ]
    ...
    async def get_pr_statistics(
        ...
        active_query = select(func.count(PullRequest.id)).where(
    ...
    async def bulk_update_last_checked(
        self, pr_ids: list[uuid.UUID], checked_at: datetime | None = None
    ) -> int:
