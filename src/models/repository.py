...
class Repository(BaseModel):
    ...
    full_name: Mapped[Optional[str]] = mapped_column(String(300), nullable=True)  # owner/repo
    ...
    last_failure_reason: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    
    # Configuration overrides
    config_override: Mapped[Optional[dict[str, Any]]] = mapped_column(JSONB, nullable=True)
    
    # GitHub API configuration
    github_token: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    github_app_id: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    ...
    repo_metadata: Mapped[Optional[dict[str, Any]]] = mapped_column("metadata", JSONB, nullable=True)
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint("url", name="uq_repository_url"),
    )
    ...
    def increment_failure_count(self, reason: Optional[str] = None) -> None:
    ...
    def get_auth_config(self) -> dict[str, Any]:
        ...
        if self.github_app_id and self.github_installation_id:
            config["app_id"] = self.github_app_id
            config["installation_id"] = self.github_installation_id
    ...
    def update_from_github_repo(self, repo_data: dict[str, Any]) -> None:
        ...
        self.repo_metadata.update({k: v for k, v in github_metadata.items() if v is not None})