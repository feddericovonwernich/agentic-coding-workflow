...
class TestSyncOperationExecution:
    ...
    async def test_execute_empty_sync_operation(self, mock_db_transaction, synchronizer):
    ...
    async def test_execute_sync_operation_failure(
        ...
        mock_db_transaction.return_value.__aenter__.side_effect = Exception("Test error")
...
class TestPRSynchronization:
    ...
    async def test_categorize_pr_operations_existing(
        ...
        synchronizer.pr_repo.get_by_repo_and_number = AsyncMock(return_value=existing_pr)
    ...
    async def test_pr_needs_update_true(self, synchronizer, sample_discovery_result):
        ...
        needs_update = await synchronizer._pr_needs_update(existing_pr, sample_discovery_result)
    ...
    async def test_create_prs_bulk_with_validation_error(
        ...
        count = await synchronizer._create_prs_bulk([invalid_pr, sample_discovery_result])
...
class TestCheckRunSynchronization:
    ...
    async def test_categorize_check_operations_all_new(
        ...
        checks_to_create, checks_to_update = await synchronizer._categorize_check_operations(
    ...
    async def test_categorize_check_operations_existing(
        ...
        synchronizer.check_run_repo.get_by_external_id = AsyncMock(return_value=existing_check)
    ...
    async def test_check_needs_update_true(self, synchronizer, sample_check_run_discovery):
    ...
    async def test_check_needs_update_false(self, synchronizer, sample_check_run_discovery):
...
class TestConflictResolution:
    ...
    async def test_update_pr_github_wins_strategy(self, synchronizer, sample_discovery_result):
...
class TestRollbackOperations:
    ...
    async def test_rollback_sync_operation_failure(self, synchronizer):
        ...
        assert result.status == OperationStatus.FAILED
        assert any("Rollback failed: Rollback error" in error for error in result.errors)
...
class TestIntegrationScenarios:
    ...
    async def test_full_synchronization_workflow(
        ...
        with patch('src.workers.monitor.synchronization.DatabaseTransaction') as mock_tx:
