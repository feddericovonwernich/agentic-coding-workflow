...
class TestPRDiscoveryEngine:
    ...
    def discovery_engine(
        self, mock_github_client, mock_cache_manager, discovery_config
    ):
    ...
    async def test_get_pull_request_details_success(
        ...
        result = await discovery_engine.get_pull_request_details(
    ...
    async def test_get_pull_request_details_cached(
        ...
        discovery_result = (
            await discovery_engine._convert_github_pr_to_discovery_result(
    ...
    async def test_get_pull_request_details_not_found(
        ...
        with pytest.raises(GitHubNotFoundError):
            await discovery_engine.get_pull_request_details(
                "octocat", "Hello-World", 999
            )

    async def test_check_pr_exists_true(
        self, discovery_engine, sample_github_pr, mock_github_client
    ):
    ...
    async def test_repository_not_found_handling(
        ...
        mock_paginator = AsyncMock()
        mock_paginator.__aiter__.side_effect = GitHubNotFoundError(
    ...
    async def test_concurrent_processing(self, mock_github_client, mock_cache_manager):
        ...
        repos = [
            RepositoryContext(uuid.uuid4(), f"owner{i}", f"repo{i}") for i in range(3)
        ]
    ...
    async def test_cache_key_generation_no_since(
        self, discovery_engine, sample_repo_context
    ):
    ...
    async def test_create_batches(self, discovery_engine):
        """Test batch creation."""
        repos = [
            RepositoryContext(uuid.uuid4(), f"owner{i}", f"repo{i}") for i in range(5)
        ]
    ...
    async def test_clear_cache_default_pattern(
        self, discovery_engine, mock_cache_manager
    ):
...
class TestCheckRunDiscoveryEngine:
    ...
    def sample_failed_check_run(self):
        ...
        return {
            ...
            "output": {
                ...
                "annotations": [
                    {"path": "src/main.js", "message": "Missing semicolon"}
                ],
                "images": [],
            },
            "app": {"name": "ESLint", "slug": "eslint"},
            "pull_requests": [],
    ...
    async def test_convert_github_check_run_success(
        ...
        result = await check_run_engine._convert_github_check_run(
            sample_github_check_run
        )
    ...
    async def test_extract_check_metadata(
        self, check_run_engine, sample_github_check_run
    ):
        """Test metadata extraction from check run."""
        metadata = await check_run_engine._extract_check_metadata(
            sample_github_check_run
        )
    ...
    async def test_process_check_suite_cached(
        ...
        cached_suite = {"id": 67890, "head_branch": "main", "status": "completed"}
    ...
    async def test_get_check_run_details_not_found(
        ...
        with pytest.raises(GitHubNotFoundError):
            await check_run_engine.get_check_run_details(
    ...
    async def test_get_failed_check_runs_mixed_results(
        ...
        mock_paginator = MockAsyncIterator(
            [sample_github_check_run, sample_failed_check_run]
        )
        mock_github_client.paginate.return_value = mock_paginator
    ...
    async def test_commit_not_found_handling(
        self, check_run_engine, mock_github_client
    ):
